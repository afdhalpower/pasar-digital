...
@section('title', __('marketplace.profile_title'))
    ...
    <div style="display: grid; grid-template-columns: 280px 1fr; gap: var(--space-5);" class="dashboard-grid">
        ...
        <main class="dashboard-content-area animate-in">
            <div style="margin-bottom: var(--space-4);">
                <span class="label-sm" style="color: var(--color-primary);">{{ __('marketplace.profile_badge') }}</span>
                <h1 class="headline-md" style="margin-top: 4px;">{{ __('marketplace.profile_heading') }}</h1>
                <p style="color: var(--color-on-surface-variant); font-size: 0.875rem;">{{ __('marketplace.profile_subtitle') }}</p>
            </div>
            ...
            <div style="background: var(--color-surface-container); border: 1px solid var(--color-outline-variant); border-radius: var(--radius-lg); padding: var(--space-4) var(--space-5);">
                ...
                @if ($errors->any())
                    <div class="alert alert-danger">
                        ...
                        <div style="display: flex; flex-direction: column;">
                            <span style="font-weight: 600;">{{ __('marketplace.profile_errors') }}</span>
                ...
                <form action="{{ route('buyer.profile.update') }}" method="POST" autocomplete="off">
                    ...
                    <div style="margin-bottom: var(--space-4);">
                        <h2 class="form-section-title">{{ __('marketplace.profile_personal_info') }}</h2>
                        
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-3);" class="dashboard-grid">
                            <div class="form-group">
                                <label for="name" class="form-label">{{ __('marketplace.profile_name') }}</label>
                            ...
                            <div class="form-group">
                                <label for="email" class="form-label">{{ __('marketplace.profile_email') }}</label>
                    ...
                    <div style="margin-bottom: var(--space-4);">
                        <h2 class="form-section-title">{{ __('marketplace.profile_change_password') }}</h2>
                        <p style="color: var(--color-on-surface-variant); font-size: 0.8rem; margin-top: -8px; margin-bottom: 16px;">
                            {{ __('marketplace.profile_password_hint') }}
                        </p>

                        <div class="form-group">
                            <label for="current_password" class="form-label">{{ __('marketplace.profile_current_password') }}</label>
                            <input type="password" name="current_password" id="current_password" class="input-field" placeholder="••••••••" autocomplete="new-password">
                            <span class="input-hint">{{ __('marketplace.profile_current_password_hint') }}</span>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-3);" class="dashboard-grid">
                            <div class="form-group">
                                <label for="new_password" class="form-label">{{ __('marketplace.profile_new_password') }}</label>
                                <input type="password" name="new_password" id="new_password" class="input-field" placeholder="{{ __('marketplace.profile_new_password_placeholder') }}">
                                @error('new_password')
                                    <span style="color: var(--color-error); font-size: 0.75rem; display: block; margin-top: 4px;">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                <label for="new_password_confirmation" class="form-label">{{ __('marketplace.profile_confirm_password') }}</label>
                                <input type="password" name="new_password_confirmation" id="new_password_confirmation" class="input-field" placeholder="{{ __('marketplace.profile_confirm_password_placeholder') }}">
                            </div>
                        </div>
                    </div>
